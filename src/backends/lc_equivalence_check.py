...
"is_lc_equivalent" function. This section is based on articles [1]_ and [2]_, please refer to them for more details.

Any n-partite stabilizer state is defined by a set of "n" commuting operators called the stabilizer generators. Each
member of this set is called a stabilizer, and consists of tesnor product of local Pauli operators on each of the
qubits. The state is the unique common eigenstate of this set with eigenvalue equal to +1. The set itself is not
unique and any set of "n" independent members of the stabilizer group, that is generated by this generator set, is also
valid. The stabilizers can be pictured in a binary representation which maps any of the 4 Pauli operations into
binary numbers

.. math::

   \sigma_0 = I = 00,\ \sigma_x = 01,\  \sigma_z = 10,\  \sigma_y = 11


This can be generalized to n-qubit case, that is :math:`\sigma_x \otimes \sigma_y \otimes \cdots \otimes \sigma_z=
\left( 0, 1, \cdots, 1| 1, 1, \cdots, 0 \right)`

which is a 2n-dimensional vector.

The generators set can then be represented by 2n*n matrix which can be thought of as two n*n square matrices on top of
each other. A column in this matrix is a 2n-dimensional vector representing one stabilizer operator.
...
The reason behind this naming convention is that if an element :math:`Z_{ij}` is equal to one, in the Z matrix,
and the corresponding element :math:`X_{ij}` is zero, in the X matrix, then this shows a Pauli Z operator acting on
qubit "i" in the j-th stabilizer operator. The other way around which is :math:`Z_{ij} = 0` and :math:`X_{ij} =1` is
a Pauli X acting on qubit "i". And if both elements are 0 or 1, they correspond to identity "I" and Pauli Y operators
respectively. For the special case of graph states, the X matrix is equal to identity "I" and the Z matrix is the
adjacency matrix of the graph usually shown by :math:`\theta`.


A general n-qubit local Clifford operation in this formalism can be represented by a 2n*2n matrix Q, made of 4 square and
diagonal matrices A, B, C and D.
 ...
 Each 2*2 sub-matrix, made of i-th diagonal elements of the A, B, C, D, acts locally on the i-th qubit in the system
...
where P is the 2n*2n matrix of the form  :math:`P =\begin{bmatrix} 0 & I \\ I & 0 \end{bmatrix}`.
This gives rise to a system of :math:`"n^2"` linear equations to find :math:`4n` unknown elements of Q
...
Remember that if the system of equations is not under-defined, then there exists no solution (other than Q=0 which is
not a valid Clifford operation) and graph states are not equivalent for sure. It is proved that a solution for a valid
...
"B_dependent". Solving for vector "x" will result in finding one of the members "b" of solution basis "B". As evident
here, the size of B is equal to the size of "B_dependent" which is equal to the number of linearly dependent columns
in the initial coefficient matrix.

If Q is found, the graphs are LC equivalent. In this case one can translate the sub-matrices :math:`Q^{(i)}` to local
Clifford operations (combination of H and P gates) needed (via "local_clifford_ops" function) to convert one graph
state to the other.
...
"lc_graph_operations" function) that transform one graph to the other one in a step by step graph transform. Please
read section IV of [2]_ for details. In order to do so one starts with
making a so called R matrix which is defined as
...
suppose that the zero diagonal is at "j-th" location and the element = 1 in that column is at "k-th" row. Then a
double :math:`f_{jk}` transformation is applied of R-matrix and added to the sequence. This is repeated until no
more eligible element can be found in the diagonal of the R-matrix.
...
def lc_graph_operations(z_1, solution):
    """
    Finds a list local complementations needed to be applied on each node of the given graph to transform into target,
        given the Clifford transformation matrix, which is the output of the solver function.
...
def iso_equal_check(graph1, graph2):
    ...
    for x in iso_z_2:
        success, solution = is_lc_equivalent(z_1, x)
...
def local_comp_graph(input_graph, node_id):
    ...
    new_adj_matrix = (adj_matrix @ (
                gamma_matrix @ adj_matrix + adj_matrix[node_id, node_id] * gamma_matrix + identity) % 2) % 2
