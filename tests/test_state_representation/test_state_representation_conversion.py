...
def test_density_to_graph_linear_cluster_3():
    ...
    Adj=np.zeros([3,3]); Adj[0,1]=1; Adj[1,2]=1; Adj=Adj+Adj.transpose();
...
def test_density_to_graph_Triangle():
    ...
    assert((graph>=0).all()) #Matrix should consist of elements greater or equal to zero
    
    #Compare calculated adjancency matrix to exprected expression
    Adj=np.zeros([3,3]); Adj[0,1]=1; Adj[0,2]=1; Adj[1,2]=1; Adj=Adj+Adj.transpose();
    assert((graph==Adj).all()) 
