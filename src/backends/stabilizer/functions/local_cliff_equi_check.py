...
def lc_check(state1, state2, validate=True):
    ...
    if validate:
        ...
        if not (z_equal and x_equal):
            warnings.warn("the gate sequence is not converting the state1's stabilizer tableau into state2's")
...
def _to_graph(state):
    ...
    if isinstance(state, nx.Graph):
        ...
        try:
            ...
            raise ValueError("the QuantumState provided has no active stabilizer representation")
        ...
        raise ValueError("input data should either be a adjacency matrix, graph, Clifford or Stabilizer tableau or a "
