...
Each member of this set is called a stabilizer, and consists of tenor product of local Pauli operators on each of the
...
def is_lc_equivalent(adj_matrix1, adj_matrix2, mode="deterministic"):
    ...
    :type mode: str
    :raises AssertionError: if the number of rows is less than the rank in the row reduced matrix or
        if the number of columns is not equal to :math:`4n - rank`
        (for :math:`n` being the number of nodes in the graph) after removing linearly dependent columns
...
def _random_checker(reduced_coeff_matrix, col_list, trial_count=10000):
    """
