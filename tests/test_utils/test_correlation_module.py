...
@pytest.mark.parametrize("metric", ["deg", "bet", "close", "eigen", "nei_deg", "eccentric", "cluster", "pagerank"])
...
def test_node_corr(metric, circ_met):
    ...
    correlation = Node_corr(rnd_graph(n=4, p=0.7), relabel_trials=2, metric=metric, circ_met=circ_met, show_plot=False)
...
@pytest.mark.parametrize("graph_metric", ["max_between", "mean_nei_deg", "mean_deg", "node_connect", "edge_connect",
                                          "assort", "radius", "center", "periphery", "cluster", "local_efficiency"])
@pytest.mark.parametrize("circ_metric", ["num_emit", "cnot", "depth"])
def test_random_graph_corr(graph_metric, circ_metric):
    correlation = Graph_corr(graph_metric=graph_metric, circ_metric=circ_metric, trials=4)
...
def test_initial_graph_corr():
    ...
    correlation = Graph_corr(graph_metric="mean_nei_deg", circ_metric="depth", initial_graph=g, num_isomorph=2)
...
def test_graph_list_corr():
    ...
    correlation = Graph_corr(graph_metric="mean_nei_deg", circ_metric="num_emit", graph_list=graph_list)
    correlation.finder(show_plot=False)
    correlation.met_met(met1="num_emit", met2="node_connect", show_plot=False)
